...
**NOTE**: *You can call your geonode project whatever you like **except 'geonode'**. Follow the naming conventions for python packages (generally lower case with underscores (``_``). In the examples below, replace ``thuenen_atlas`` with whatever you would like to name your project.*
...
1. Generate the project
    ...
    mkvirtualenv --python=/usr/bin/python3 thuenen_atlas
    pip install Django==3.2.14

    django-admin startproject --template=./geonode-project -e py,sh,md,rst,json,yml,ini,env,sample,properties -n monitoring-cron -n Dockerfile thuenen_atlas
...
```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./thuenen_atlas/br/backup.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4thuenen_atlas sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./thuenen_atlas/br/backup.sh $BKP_FOLDER_NAME'
```

### Run a Restore

```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./thuenen_atlas/br/restore.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4thuenen_atlas sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./thuenen_atlas/br/restore.sh $BKP_FOLDER_NAME'
```
